...
def create_model_optimizer(args: Namespace) -> (
    SSMILVAE.SSMILVAE, Adam
):
    # If the mode is equal to base attention
    if args.model == 'base_att':
        # Create the Attention MIL model based on the given dataset
        model = AttentionMILClassifier.AttentionBase_MNIST().to(device=args.device) if args.data == 'MNIST' \
    ...
    # Otherwise
        ...
        if args.model == 'sepopt':
            ...
            optimizer_classifier = Adam(model.att.parameters(), lr=0.0001,
                                        betas=(0.9, 0.999), weight_decay=0.0005)
            # Declare the optimizer
            optimizer_vae = Adam(list(model.enc.parameters()) + list(model.dec.parameters()),
                                 lr=args.lr,
                                 betas=(0.9, 0.999),
                                 weight_decay=args.wd)
            # Declare the multiple optimizers object
            optimizer = SSMILVAE.MultipleOptimizer(optimizer_vae,
...
def main(model: SSMILVAE.SSMILVAE,
         args: Namespace,
         optimizer: Adam,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.full_data_train:
            ...
            loss_train, loss_VAE_train, acc_train = validate_full(model, args,
                                                                  trainloader,
                                                                  loadername='-Train-',
                                                                  train=True)
            # Run validation [for validation set]
            loss_val, loss_VAE_val, acc_val = validate_full(model, args,
                                                            validationloader,
        ...
        # Otherwise
            ...
            train(model, args, optimizer,
                  labeledloader, unlabeledloader)
            # Run validation [for training set]
            loss_train, loss_VAE_train, acc_train = validate(model, args,
                                                             trainloader,
                                                             loadername='-Train-',
                                                             train=True)
            # Run validation [for validation set]
            loss_val, loss_VAE_val, acc_val = validate(model, args,
                                                       validationloader,
                                                       loadername='-Validation-',
                                                       train=False)
    ...
    for modelname in os.listdir(f'{args.MODELPATH}'):
        ...
        if args.full_data_train:
            # Run test
            loss_test, loss_VAE_test, acc_test = validate_full(model, args,
                                                               testloader,
                                                               loadername=f'-Test[{modelname}]-',
                                                               train=True)
        # Otherwise
        else:
            # Run test
            loss_test, loss_VAE_test, acc_test = validate(model, args,
                                                          testloader,
                                                          loadername=f'-Test[{modelname}]-',
                                                          train=True)
    ...
    for i in range(1, args.numsamp + 1):
        sample_image(args=args, model=model,
                     idx=i, epoch=None)

    # Return the results
    return losses_train, losses_VAE_train, accs_train, \
        losses_val, losses_VAE_val, accs_val, \
        loss_test, loss_VAE_test, acc_test


def main_base(model: SSMILVAE.SSMILVAE,
              args: Namespace,
              optimizer: Adam,
              trainloader: DataLoader,
              validationloader: DataLoader,
    ...
    losses_train = list()
    losses_val = list()
    args.prev_val = -1. if args.model == 'base_att' else 1e9
    # Declare real image counter to save
    args.real_image_counter = 0
    # Trace args
    print(args)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Run validation [for training set]
        loss_train = validate_base(model, args,
                                   trainloader,
                                   loadername='-Train-')
        # Run validation
        ...
        losses_train.append(loss_train)
        # Run validation [for validation set]
        loss_val = validate_base(model, args,
                                 validationloader,
                                 loadername='-Validation-',
                                 train=False)
        # Record validation results
    ...
    args.max_test_acc = -1. if args.model == 'base_att' else 1e9
    # Test each one of the saved model to find the best
    for modelname in os.listdir(f'{args.MODELPATH}'):
        ...
        model.load_state_dict(load(f'{args.MODELPATH}/{modelname}'))
        # Run test
        loss_test = validate_base(model, args,
                                  testloader,
                                  loadername=f'-Test[{modelname}]-',
                                  train=True)
...
if __name__ == '__main__':
    ...
    # Declare paths
    args.TRAININGPATH = f'./records/train-test/{args.data}/{args.mode}/{args.TIME}/{args.model}' if not args.test_mode \
        else f'./records/train-test/{args.data}/{args.mode}/{args.TIME}-test/{args.model}'
    args.RECONPATH = f'./records/reconstructions/{args.data}/{args.mode}/{args.TIME}/{args.model}' if not args.test_mode \
        else f'./records/reconstructions/{args.data}/{args.mode}/{args.TIME}-test/{args.model}'
    args.MODELPATH = f'./records/models/{args.data}/{args.mode}/{args.TIME}/{args.model}' if not args.test_mode \
    ...
    for path in [getattr(args, p) for p in ['TRAININGPATH', 'RECONPATH', 'MODELPATH', 'LOADERPATH']]:
        if not os.path.exists(path):
            os.makedirs(path)

    # Load the DataLoaders
    trainloader, labeledloader, unlabeledloader, \
        validationloader, testloader = load_data(args=args)
    # Trace
    ...
    if args.model == 'base':
        ...
        losses_train, losses_val = main_base(model,
                                             args,
                                             optimizer,
                                             trainloader,
                                             validationloader,
        ...
        np.save(file=f"{args.TRAININGPATH}/loss-acc_results.npy",
                arr=np.array([
                     losses_train,
                     losses_val,
                ]),
    ...
    # Otherwise
        ...
        losses_train, losses_VAE_train, accs_train, \
            losses_val, losses_VAE_val, accs_val, \
            _, _, _ = main(model,
                           args,
                           optimizer,
